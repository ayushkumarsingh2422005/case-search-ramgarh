...
export async function GET(request: NextRequest) {
  try {
    ...
    const ioParam = searchParams.get('investigatingOfficer');
    if (ioParam === '__none__') {
      filter.$or = [{ investigatingOfficer: { $exists: false } }, { investigatingOfficer: null }, { investigatingOfficer: '' }];
    } else if (ioParam) {
      filter.investigatingOfficer = ioParam;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.investigatingOfficer === '' || body.investigatingOfficer === undefined) {
      delete body.investigatingOfficer;
    }
